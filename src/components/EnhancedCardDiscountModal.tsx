// components/EnhancedCardDiscountModal.tsx
// Enhanced version with ML ranking integration

import React, { useState, useEffect } from "react";
import { FaCreditCard, FaClock, FaPercent, FaCheckCircle, FaArrowLeft, FaTimesCircle, FaExclamationTriangle, FaStar, FaTrophy, FaBrain } from "react-icons/fa";
...
const EnhancedCardDiscountModal: React.FC<CardDiscountModalProps> = ({
  isOpen,
  onClose,
  products,
  totalAmount,
  ...
  const loadAvailableCards = async (requestType: 'immediate' | 'scheduled', isManualReload = false) => {
    ...
    try {
      ...
      // Use the enhanced API endpoint with ML ranking
      const endpoint = useMLRanking ? '/api/cardholder/available-cards-ranked' : '/api/cardholder/available-cards';
      ...
      if (response.ok) {
        ...
        // Extract ranking information for display
        if (cards.length > 0 && cards[0].ranking) {
          setRankingInfo({
            mlRanked: cards[0].ranking.mlRanked,
            processingTime: cards[0].ranking.processingTime,
            totalCards: cards.length
          });
        }
        
        console.log(`Loaded ${cards.length} available cards`, { 
          mlRanked: useMLRanking, 
          topHealthScore: cards[0]?.ranking?.healthScore 
        });
      } else {
  ...
  const getRankBadge = (rank: number) => {
    ...
    if (rank <= 3) return <FaStar className="text-yellow-400" />;
    if (rank <= 5) return <BsShieldCheck className="text-blue-500" />;
    return null;
  };
  ...
  const toggleRankingMode = () => {
    setUseMLRanking(!useMLRanking);
    setRankingInfo(null);
    // Reload cards with new ranking mode
    loadAvailableCards(step === 'immediate' ? 'immediate' : 'scheduled', true);
  };
        ...
        <div className={`${step === 'declined' ? 'bg-red-600' : 'bg-amazon_blue'} text-white p-4 rounded-t-lg`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-semibold flex items-center space-x-2">
                ...
                <span>
                  {step === 'declined' ? 'Payment Request Declined' : 'Smart Card Matching'}
                </span>
                {useMLRanking && rankingInfo?.mlRanked && (
                  <FaBrain className="text-yellow-300" title="AI-Powered Ranking" />
                )}
              </h2>
            </div>
            <button
          ...
          <p className="text-sm mt-2 text-gray-200 ml-10">
            {step === 'declined' 
              ...
              : 'AI-powered matching finds the best cardholders for your discount needs'
            }
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-xs text-green-600 mt-2">
                    ✓ Faster processing • ✓ Real-time matching • ✓ AI-ranked results
                  </p>
                </button>
                ...
                >
                  ...
                  <p className="text-xs text-blue-600 mt-2">
                    ✓ More options • ✓ 30 min validity • ✓ Smart ranking
                  </p>
                </button>
              </div>
            </div>
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={toggleRankingMode}
                      className="rounded"
                    />
                    <label htmlFor="mlRanking" className="text-sm flex items-center space-x-1">
              ...
              {rankingInfo && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center space-x-2 text-sm">
                    <FaBrain className="text-blue-500" />
                    <span className="font-semibold">Smart Ranking Active:</span>
                    <span>
                      {rankingInfo.mlRanked ? (
                        <>ML-powered ranking ({rankingInfo.processingTime?.toFixed(1)}ms)</>
                      ) : (
                        <>ML-powered ranking</>
                      )}
                    </span>
                    <span className="text-gray-600">• {rankingInfo.totalCards} cards evaluated</span>
                  </div>
                </div>
              )}
                        ...
                        >
                          ...
                          {card.ranking && (
                            <div className="absolute top-2 right-2 flex items-center space-x-2">
                              {getRankBadge(card.ranking.rank)}
                              ...
                              <div>
                                ...
                                {card.ranking && (
                                  <div className="flex items-center space-x-2 mt-1">
                                    ...
                                    <span className={`text-xs font-semibold ${getHealthScoreColor(card.ranking.healthScore)}`}>
                                      {(card.ranking.healthScore * 100).toFixed(0)}% ({getHealthScoreLabel(card.ranking.healthScore)})
                                    </span>
                                    {card.ranking.mlRanked && (
                                      <FaBrain className="text-xs text-blue-400" title="AI-Generated Score" />
                                    )}
                                  </div>
                                )}
                              </div>
                          ...
                          <div className="mt-3 flex items-center justify-between text-sm">
                            ...
                            <button className="text-amazon_blue hover:underline font-medium">
                              Select →
                            </button>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      {selectedCard.ranking && (
                        <p className="text-xs text-gray-500">
                          Ranked #{selectedCard.ranking.rank} • Trust Score: {(selectedCard.ranking.healthScore * 100).toFixed(0)}%
                        </p>
                      )}
                    </div>
                  </div>
                  {selectedCard.ranking && getRankBadge(selectedCard.ranking.rank)}
                </div>
              </div>
            ...
            <div className="text-center py-8">
              ...
              {selectedCard?.ranking && (
                <p className="text-sm text-gray-500">
                  Selected cardholder ranked #{selectedCard.ranking.rank} with {(selectedCard.ranking.healthScore * 100).toFixed(0)}% trust score
                </p>
              )}
...
export default EnhancedCardDiscountModal;